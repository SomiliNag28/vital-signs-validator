...
class TestReportEndpoint:
    ...
    def test_nonexistent_report_returns_404(self):
        """Requesting a guaranteed nonexistent report returns 404."""
        # Use timestamp-based name — guaranteed to never exist
        import time
        fake_name = f"report_does_not_exist_{int(time.time())}.html"
        response = client.get(f"/report/{fake_name}")
        assert response.status_code == 404

    def test_path_traversal_rejected(self):
        """Path traversal attempt is blocked."""
        # TestClient encodes .. so we test the validation logic directly
        from api.main import app
        bad_names = ["..etc_passwd.html", "....passwd.html"]
        for name in bad_names:
            response = client.get(f"/report/{name}")
            # Either blocked (400) or not found (404) — both are safe
    ...
    def test_generated_report_is_downloadable(self):
        ...
        assert any(t in content_type
                   for t in ["text/html", "application/json"])