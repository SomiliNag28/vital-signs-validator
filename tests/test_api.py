...
class TestReportEndpoint:
    ...
    def test_nonexistent_report_returns_404(self):
        ...
        import time
        fake_name = f"validation_report_{int(time.time())}_fake.html"
        response = client.get(f"/report/{fake_name}")
        assert response.status_code == 404

    def test_path_traversal_rejected(self):
        """Non-validation-report filename is rejected."""
        response = client.get("/report/random_file.html")
        assert response.status_code == 404

    def test_generated_report_is_downloadable(self):
        """Report generated by /validate is downloadable."""
